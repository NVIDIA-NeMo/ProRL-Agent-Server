...
def test_balance_all_correct_subsamples_to_mixed_count() -> None:
    ...
    assert len(selected) == 4  # 2 mixed + 2 sampled all-correct
...
def test_full_pipeline_drop_then_balance() -> None:
    groups = (
        [_group(0, [0.0, 0.0])]  # all-wrong -> dropped by D
        + [_group(1, [1.0, 0.0], actions=["keep", "keep"])]  # mixed, no switch -> dropped by B
    ...
    # D drops group0, B drops group1 -> 1 mixed + 4 all-correct; C downsamples
    # all-correct to the mixed count (1) -> 1 mixed + 1 all-correct.
    assert len(selected) == 2
