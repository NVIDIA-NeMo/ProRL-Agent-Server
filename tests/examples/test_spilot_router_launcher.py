...
def test_spilot_agent_template_builds_registered_harness() -> None:
    ...
    text = text.replace("${SPILOT_ROUTER_MEMORY}", "full")
    text = text.replace("${SPILOT_COST_PENALTY_MODE}", "multiplicative")
    text = text.replace("${SPILOT_DIFFICULTY_LEDGER_PATH}", "")
    text = text.replace("${SPILOT_DIFFICULTY_EASY_MULT}", "1.0")
    text = text.replace("${SPILOT_DIFFICULTY_HARD_MULT}", "1.0")
