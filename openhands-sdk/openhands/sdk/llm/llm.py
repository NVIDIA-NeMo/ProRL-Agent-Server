...
class LLM(BaseModel, RetryMixin):
    ...
    def metrics(self) -> Metrics:
        ...
        Returns:
            Metrics object containing token usage statistics.
    ...
    def completion(
        ...
        **kwargs,
        ...
        kwargs["tools"] = cc_tools if cc_tools else None
        has_tools_flag = bool(cc_tools)
    ...
    def is_caching_prompt_active(self) -> bool:
        ...
        if not self.caching_prompt:
            return False
        # Prompt caching is supported for Anthropic Claude models
        # LiteLLM handles the actual caching implementation
        model_for_caps = self._model_name_for_capabilities().lower()
        return self.caching_prompt and "claude" in model_for_caps
    ...
    def format_messages_for_llm(self, messages: list[Message]) -> list[dict]:
        ...
        for message in messages:
            ...
            message.function_calling_enabled = True  # Always use native function calling
            # Use explicit user config for force_string_serializer
            # If not set, default to False (most models support structured content)
            message.force_string_serializer = (
                self.force_string_serializer
                if self.force_string_serializer is not None
                else False
            )
            # send_reasoning_content is now disabled by default
            # Users can enable via system prompt or custom message handling
