...
class Entity(BaseModel):
    ...
    def prompt_repr(self) -> str:
        """Return a human-readable representation of the entity used in prompts."""
        if self.type in (EntityType.FUNCTION, EntityType.CLASS, EntityType.METHOD):
            return (
                f"{self.type.value} '{self.name}' -- {self.file_name}:{self.start_lineno}-{self.end_lineno}"
            )
        return f"{self.type.value} -- {self.file_name}:{self.start_lineno}"
...
class FileDiff(BaseModel):
    ...
    def get_patch(self) -> str:
        ...
        if self.is_binary_file and self.binary_line is not None:
...
class ParsedCommit(BaseModel):
    ...
    # Duplicate commit_date property removed to avoid mypy errors
